...
func (lbaas *LbaasV2) createFullOctaviaLoadBalancer(name, clusterName string, service *corev1.Service, nodes []*corev1.Node, svcConf *serviceConfig) (*loadbalancers.LoadBalancer, error) {
	createOpts := loadbalancers.CreateOpts{
		Name:        name,
		Description: fmt.Sprintf("Kubernetes external service %s/%s from cluster %s", service.Namespace, service.Name, clusterName),
		Provider:    lbaas.opts.LBProvider,
	...
	for _, port := range service.Spec.Ports {
		...
		poolCreateOpt := lbaas.buildPoolCreateOpt(name, string(listenerCreateOpt.Protocol), members, port, service, svcConf)
		listenerCreateOpt.DefaultPool = &poolCreateOpt
		createOpts.Listeners = append(createOpts.Listeners, listenerCreateOpt)

		more := ""
		if poolCreateOpt.Monitor != nil {
			more = " and healthmonitor"
		}
		klog.V(2).Infof("Loadbalancer %s: adding pool %s using protocol %s with %d members%s", name, poolCreateOpt.Name, poolCreateOpt.Protocol, len(newMembers), more)
	}
...
func (lbaas *LbaasV2) ensureOctaviaPool(lbName, lbID string, listener *listeners.Listener, service *corev1.Service, port corev1.ServicePort, nodes []*corev1.Node, svcConf *serviceConfig) (*v2pools.Pool, error) {
	...
	if pool == nil {
		createOpt := lbaas.buildPoolCreateOpt(lbName, listener.Protocol, members, port, service, svcConf)
...
func (lbaas *LbaasV2) buildPoolCreateOpt(lbName string, listenerProtocol string, members []v2pools.BatchUpdateMemberOpts, port corev1.ServicePort, service *corev1.Service, svcConf *serviceConfig) v2pools.CreateOpts {
	...
	var monitor *v2monitors.CreateOpts
	if svcConf.enableMonitor {
		opts := lbaas.buildMonitorCreateOpts(lbName, port)
		monitor = &opts
	}
...
func (lbaas *LbaasV2) ensureOctaviaListener(lbName, lbID string, oldListeners []listeners.Listener, service *corev1.Service, port corev1.ServicePort, svcConf *serviceConfig) (*listeners.Listener, error) {
...
func (lbaas *LbaasV2) ensureOctaviaLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service, nodes []*corev1.Node) (*corev1.LoadBalancerStatus, error) {
	...
	if err != nil {
		...
		klog.V(2).Infof("Creating full loadbalancer %s for Service %s", name, serviceName)
		loadbalancer, err = lbaas.createFullOctaviaLoadBalancer(name, clusterName, service, nodes, svcConf)
		if err != nil {
	...
	if !justCreated {
		...
		for _, port := range service.Spec.Ports {
			listener, err := lbaas.ensureOctaviaListener(name, loadbalancer.ID, listenersCopy, service, port, svcConf)
			if err != nil {
				return nil, err
			}
